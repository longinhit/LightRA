...
def multi_head_attention_forward(
    ...
    custom_hook_func = None
    if isinstance(query, List):
        custom_hook_func = query[1]
        query = query[0]
    # set up shape vars
    ...
    if custom_hook_func is not None:
        attn_output_prefix = custom_hook_func(q, dropout_p, is_causal)
...
class MultiheadAttention(Module):
    ...
    def forward(
        ...
        if custom_hook_func is None:
            ...
            attn_output, attn_output_weights = multi_head_attention_forward(
                [query, custom_hook_func], key, value, self.embed_dim, self.num_heads,
                self.in_proj_weight, self.in_proj_bias,
                None, None, False,
                0., self.out_proj.weight, self.out_proj.bias,
                training=self.training,
